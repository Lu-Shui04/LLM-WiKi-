"""旧的向量检索对话流。`chat_backend=legacy` 时由 app/api/chat.py 调进来。

SSE 事件和 wiki 链路完全一致（start / sources / thinking / token / usage / done /
error），所以两条链路共用同一个前端：换掉的只是「知识从哪来」。

为什么被换掉：相关性裁决发生在**查询那一刻**，手上只有一个余弦分数，而这个库上
正负样本在分数上是重叠的——正样本最低 0.509（「你平时怎么学习的」）低于负样本
最高 0.589（「帮我写一段 Python 快排」，代码题天然贴近技术语料）。没有阈值能把
它们分开，于是只能在上面堆规则：9 条固定栏目路由 + 意图分类 + 寒暄拦截 +
联系方式短路，等于用规则手工重建一个本该由知识组织方式解决的问题。详见 vectors.py。
"""
import re
...
from app.api.sse import usage_event as _usage_event
...
FIXED_SECTIONS: list[tuple[str, re.Pattern, list[str], str | None, bool]] = [
    ...
    # 0.548、项目切片 0.541，差 0.007，就是在瞎蒙（详见 app/legacy/vectors.py 里对检索边界的说明）。
    # 问法和答案都固定，直接规则接住。
    ("项目",
        ...
        async with session.lock:
            ...
            async for chunk in get_chat_model(streaming=True).astream(messages):
                if getattr(chunk, "usage_metadata", None):
                    usage = chunk.usage_metadata     # 最后一块只带账单，没有正文
                    continue
